...
export function randInArray(arr) {
...
export function nextFace(l, h) {
    if (l < h) {
        if (l <= 0.3)
            return randInArray(loveL);
        if (l <= 0.6)
            return randInArray(loveM);
        return randInArray(loveH);
    }
    if (l <= 0.3)
        return randInArray(healthL);
    if (l <= 0.6)
        return randInArray(healthM);
    return randInArray(healthL);
}
...
export class Data {
    ...
    set love(val) {
        this._love = val;
        document.querySelector("#love .bar").style.width = `${val * 100}%`;
    }
    get health() {
        return this._health;
    }
    set health(val) {
        this._health = val;
        document.querySelector("#battery .bar").style.width = `${val * 100}%`;
    }
}
